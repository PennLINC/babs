...
class BABS:
    ...
    def __init__(self, project_root, type_session, type_system):
        ...
        participants_file_rel: str
            Path to the list of final included subjects (and sessions) TSV file.
            This is relative to `analysis` folder.
        participants_file_abs: str
            Absolute path of `participants_file_rel`.
            Example: '/path/to/analysis/code/sub_final_inclu.tsv' for singl-ses dataset;
                '/path/to/analysis/code/sub_ses_final_inclu.tsv' for multi-ses dataset.
        job_status_path_rel: str
            Path to the `job_status.tsv` file.
            This is relative to `analysis` folder.
        job_status_path_abs: str
            Absolute path of `job_status_path_abs`.
            Example: '/path/to/analysis/code/job_status.tsv'
        job_submit_path_abs: str
            Absolute path of `job_submit_path_abs`.
            Example: '/path/to/analysis/code/job_submit.tsv'
        """
        ...
        if self.type_session == 'single-ses':
            self.list_sub_path_rel = 'code/sub_final_inclu.tsv'
        elif self.type_session == 'multi-ses':
            self.list_sub_path_rel = 'code/sub_ses_final_inclu.tsv'
        self.list_sub_path_abs = op.join(self.analysis_path, self.list_sub_path_rel)

        self.job_status_path_rel = 'code/job_status.tsv'
        self.job_status_path_abs = op.join(self.analysis_path, self.job_status_path_rel)
        self.job_submit_path_abs = op.join(self.analysis_path, 'code/job_submit.tsv')
    ...
    def babs_bootstrap(
        ...
        # not to track `job_status.tsv`:
        gitignore_file.write('\n' + 'code/job_status.tsv')
        gitignore_file.write('\n' + 'code/job_status.tsv.lock')
        ...
        self.datalad_save(
            path='code/*.tsv', message='Record of inclusion/exclusion of participants'
        )
    ...
    def babs_check_setup(self, input_ds, flag_job_test):
        ...
        if self.type_session == 'single-ses':
            list_files_code.append('sub_final_inclu.tsv')
        else:
            list_files_code.append('sub_ses_final_inclu.tsv')
    ...
    def babs_submit(self, count=1, df_job_specified=None):
        ...
        # `create_job_status_tsv(self)` has been called in `babs_status()`
        #   in `cli.py`
        ...
        try:
            with lock.acquire(timeout=5):  # lock the file, i.e., lock job status df
                df_job = read_job_status_tsv(self.job_status_path_abs)
                ...
                if df_job_submit.shape[0] > 0:
                    ...
                    # Update `analysis/code/job_submit.tsv` with new status
                    ...
                    # Update `analysis/code/job_status.tsv` with new status
                    ...
                    print(
                        ...
                        + " rows of 'analysis/code/job_status.tsv':"
                    )
