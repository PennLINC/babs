...
class BABS():
    ...
    def babs_check_setup(self, input_ds, flag_job_test):
        ...
        for i_ds in range(0, input_ds.num_ds):
            ...
            # ROADMAP: check if input ds is up-to-date?

            # check if any changes not saved in input dataset:
            input_ds_statuses = set([status['state'] for status in
                                    datalad_ds_input_ds.status()])
            # statuses should be all "clean", without anything else e.g., "modified":
            # TODO: ask Sydney:
            # 1) warning or error?
            # 2) what recommend to handle modified/unchanged?
            assert input_ds_statuses == set(["clean"]), \
                "Input dataset #" + str(i_ds + 1) \
                + " '" + input_ds.df["name"][i_ds] + "' " \
                + " status is not clean." \
                + " There might be untracked or modified files in it." \
                + " Please go to this directory: '" + self.analysis_path + "'\n" \
                + " and run `datalad status` to check what were changed." \
                + " Please clean the dataset until `datalad status` says 'nothing to save'."
        print(CHECK_MARK + " All good!")
        ...
        if not flag_job_test:
            ...
            # # datalad save `check_env.yaml`:
            # self.datalad_save(path="code/check_setup/check_env.yaml",
            #                   message="Save gathered information of designated environment")
